...
def mot_attention(
    ...
    """
    Attention helper for Mix-of-Transformers (MOT).

    - Designed for the common case where flash attention is unavailable.
    - Supports additive `attn_mask` and padding masks derived from `q_lens` / `k_lens`.

    Args:
        q,k,v: [B, L, N, D]
        q_lens,k_lens: [B] valid lengths. If None, treated as full length.
        attn_mask: additive mask broadcastable to [B, 1, Lq, Lk] or [B, N, Lq, Lk].
                  Use 0 for keep, -inf for masked.
    Returns:
        out: [B, Lq, N, D]
    """
    # We intentionally do not route to flash-attn here; MOT relies on a general mask.
    q = q.transpose(1, 2).to(dtype)  # [B, N, Lq, D]
    k = k.transpose(1, 2).to(dtype)  # [B, N, Lk, D]
    v = v.transpose(1, 2).to(dtype)  # [B, N, Lk, D]
    ...
    if q_lens is not None or k_lens is not None:
        if q_lens is None:
            q_lens = torch.full((b,), lq, device=q.device, dtype=torch.long)
        if k_lens is None:
            k_lens = torch.full((b,), lk, device=k.device, dtype=torch.long)

        qi = torch.arange(lq, device=q.device).view(1, 1, lq, 1)
        ki = torch.arange(lk, device=q.device).view(1, 1, 1, lk)
        q_valid = qi < q_lens.view(b, 1, 1, 1)
        k_valid = ki < k_lens.view(b, 1, 1, 1)
        valid = q_valid & k_valid  # [B,1,Lq,Lk]

        final_mask = torch.zeros((b, 1, lq, lk), device=q.device, dtype=q.dtype)
        final_mask = final_mask.masked_fill(~valid, float('-inf'))
